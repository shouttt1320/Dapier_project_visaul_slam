...
CURRENT_TIME=$(date +%s)
ssh user@192.168.0.148 "echo turtlebot | sudo -S date -s @$CURRENT_TIME" > /dev/null 2>&1
...
echo "==> [3/3] Waiting 4 seconds for nodes to initialize..."
sleep 4

echo "==> Status Check on Raspberry Pi:"
ssh user@192.168.0.148 "ps aux | grep -E 'turtlebot3_ros|astra_camera_node' | grep -v grep"

echo ""
echo "Robot is ready for Visual SLAM and Teleop!"
