...
DOCKING_ACTIVE_STATES = (
    'SEARCH_MARKER', 'CHECK_STAGING', 'BACKUP_STANDOFF', 'BACKUP_PREPARE', 'BACKUP_ALIGN_PREPARE',
    'TURN_LATERAL', 'LATERAL_CRUISE', 'TURN_FACE_DESK',
    'CARROT_ALIGN', 'STANDOFF_SETTLE', 'ALIGN_PARALLEL', 'CARROT_PURSUIT_ALIGN', 'VERIFY_DUAL_ALIGN',
...
class PrecisionApproacherNode(Node):
    def __init__(self):
        ...
        # ----------------------------------------------------------------------
        self.declare_parameter('docking_mode', 'marker')    # 'marker' (Astra S ArUco) or 'depth_edge' (OS30A RANSAC)
        ...
        self.declare_parameter('carrot_lookahead', 0.22)   # 22cm lookahead on normal axis
        ...
        self.declare_parameter('astra_y_base', 0.000)
        self.declare_parameter('astra_z_base', 0.134)
        self.declare_parameter('astra_pitch', 0.0)

        # ----------------------------------------------------------------------
        # Parameters: OS30A 3D Depth Servoing (Legacy / Depth Edge Mode)
        # ----------------------------------------------------------------------
        self.declare_parameter('depth_topic', '/os30a/camera/depth/image_raw')
        self.declare_parameter('cam_pitch', 1.1344)        # 65 deg downward
        self.declare_parameter('cam_x_base', 0.050)
        self.declare_parameter('cam_z_base', 0.400)
        self.declare_parameter('table_z_min', 0.090)
        self.declare_parameter('table_z_max', 0.125)
        self.declare_parameter('min_cliff_drop', 0.045)
        self.declare_parameter('floor_z_max', 0.055)
        self.declare_parameter('ransac_iterations', 20)
        ...
        self.cam_x_base = float(self.get_parameter('cam_x_base').value)
        self.cam_z_base = float(self.get_parameter('cam_z_base').value)
        self.table_z_min = float(self.get_parameter('table_z_min').value)
        self.table_z_max = float(self.get_parameter('table_z_max').value)
        self.min_cliff_drop = float(self.get_parameter('min_cliff_drop').value)
        ...
        self.backup_substate = 'ALIGN'        # Substate within BACKUP_STANDOFF: 'ALIGN' -> 'REVERSE'
        ...
        # Lateral Maneuver Variables (90° In-place turn & Cruise)
        ...
        self.dual_align_stable_required = 4
    ...
    def depth_callback(self, msg: Image):
        try:
            ...
            # In marker mode: depth is cached for 3D box plane fitting, edge processing not needed
            if self.docking_mode == 'marker':
                return

            if self.docking_mode != 'depth_edge':
                return
            if not self.calibration_mode and self.state in ('INIT', 'WAIT_NAV2', 'NAV2_READY'):
                return
            ...
            if self.state in ('TURN_LATERAL', 'LATERAL_CRUISE', 'TURN_FACE_DESK'):
                ...
                self.target_obj = TargetObject()
                self.render_and_publish_visualization(None, None, None, depth_m=depth_m)
                return

            self.process_depth_for_desk_edge(depth_m)
        except Exception as e:
            self.get_logger().warn(f"Depth processing error: {e}", throttle_duration_sec=2.0)
    ...
    def fit_depth_plane(self, marker_corners, z_marker, n_pnp):
        ...
        """
        if self.last_depth_img is None or (time.time() - self.last_depth_time) > 1.5:
            self.plane_roi_rect = None
            return None, 0
    ...
    def process_marker_detection(self, color_img: np.ndarray, frame_id: str):
        ...
        if len(active_ids) >= 2:
            ...
            # Validate physical span (e.g. 8cm ~ 50cm)
            if 0.08 <= span <= 0.50:
                # 3D midpoint of the two markers (box center)
                ...
                if p_base is not None and n_base is not None:
                    ...
                    self.docking_alignment_mode = f"DUAL:{span*100:.0f}cm"
                    self.plane_fit_source = f"DUAL:{int(span*100)}cm"
    ...
    def _control_loop_impl(self):
        ...
        # ----------------------------------------------------------------------
            ...
            if self.desk_distance is not None:
                ...
                # 1. Clearance Check: If too close (< 35cm) where marker starts clipping from FOV, back up to 45cm
                if self.desk_distance < 0.35 and (e_y_val > 0.020 or e_theta_val > math.radians(2.5)):
                    ...
                    self.get_logger().info(
                        ...
                        f"Backing up to 45cm optimal long-range staging zone..."
                    )
                    self.transition_to('BACKUP_STANDOFF')
                    return

                # 2. Unified Staging Transition:
                # Directly transition to continuous adaptive CARROT_ALIGN (eliminates disjoint P-controller!)
                self.target_lateral_offset = e_y if e_y is not None else 0.0
                self.get_logger().info(
                    f">>> [CHECK_STAGING] Target verified (dist={self.desk_distance*100:.1f}cm, ey={self.target_lateral_offset*100:+.1f}cm, yaw={math.degrees(e_theta_val):+.1f}°). "
                    f"Transitioning to unified adaptive CARROT_ALIGN..."
                )
                self.dual_align_stable_count = 0
        ...
        # ----------------------------------------------------------------------
        # STATE: BACKUP_STANDOFF (Phase 1: Align parallel to normal -> Phase 2: Reverse straight)
        # ----------------------------------------------------------------------
        elif self.state in ('BACKUP_STANDOFF', 'BACKUP_PREPARE', 'BACKUP_ALIGN_PREPARE'):
            elapsed = (now - self.state_start_time).nanoseconds / 1e9

            # Phase 1: In-place pivot to face desk normal strictly parallel (e_theta -> 0)
            if self.backup_substate == 'ALIGN':
                twist.linear.x = 0.0
                e_x, e_y, e_theta = self.compute_docking_errors()
                if e_theta is not None:
                    if abs(e_theta) <= math.radians(2.5) or elapsed > 3.0:
                        self.get_logger().info(
                            f">>> [BACKUP_STANDOFF] Heading aligned to normal axis (yaw_err={math.degrees(e_theta) if e_theta is not None else 0:+.1f}°). "
                            f"Beginning straight reverse retreat..."
                        )
                        self.backup_substate = 'REVERSE'
                        self.state_start_time = now
                        self.stop_robot()
                        return
                    else:
                        w_z = math.copysign(min(0.22, max(0.06, 0.70 * abs(e_theta))), e_theta)
                        twist.angular.z = float(w_z)
                        self.get_logger().info(
                            f">>> [BACKUP_ALIGN] In-place aligning to normal: yaw_err={math.degrees(e_theta):+.1f}°, wz={w_z:+.2f}rad/s",
            ...
            # Phase 2: Reverse straight along normal axis (Cut in half: ~7.5cm retreat)
            elif self.backup_substate == 'REVERSE':
                twist.linear.x = -0.065  # -6.5 cm/s straight reverse
                twist.angular.z = 0.0
                # Cut backup distance in half: 1.2s (~7.8cm) and desk_distance >= 0.45m (max 2.2s)
                if elapsed >= 1.2 and ((self.desk_distance is not None and self.desk_distance >= 0.45) or elapsed > 2.2):
                    self.get_logger().info(">>> Half-distance standoff secured along normal axis (~7.5cm retreat). Re-assessing staging...")
                    self.stop_robot()
                    self.transition_to('CHECK_STAGING')
                    return
        ...
        # ======================================================================
            ...
            if curr_pose is not None:
                ...
                if abs(yaw_err) <= math.radians(2.5):
                    ...
                    if self.stable_yaw_count >= 3:
                        self.get_logger().info(">>> [TURN_LATERAL] 90° Turn Complete! Heading aligned with lateral axis.")
                        self.stop_robot()
                ...
                elapsed = (now - self.state_start_time).nanoseconds / 1e9
                twist.angular.z = 0.20 if self.target_lateral_offset > 0 else -0.20
                if elapsed >= 7.85:
                    self.stop_robot()
                    self.transition_to('LATERAL_CRUISE')
                    return
            ...
            if self.lateral_traveled >= (dist_target - 0.010):
                self.get_logger().info(">>> [LATERAL_CRUISE] Target displacement reached! Preparing to face desk...")
                self.stop_robot()
                self.transition_to('TURN_FACE_DESK')
                return

            twist.linear.x = 0.040
            twist.angular.z = 0.0
            ...
            if curr_pose is not None:
                ...
                if abs(yaw_err) <= math.radians(2.5):
                    ...
                    twist.angular.z = 0.0
                    if self.stable_yaw_count >= 3:
                        self.get_logger().info(f">>> [TURN_FACE_DESK] 90° Turn Complete! Re-faced desk (yaw_err={math.degrees(yaw_err):+.1f}°).")
                        self.stop_robot()
                        self.transition_to('CHECK_STAGING')
                        return
                else:
        ...
        # ======================================================================
        # CASE A: Carrot Pursuit Alignment (CARROT_ALIGN -> ALIGN_PARALLEL)
        # ======================================================================
            ...
            if e_x is not None and e_y is not None:
                ...
                elapsed = (now - self.state_start_time).nanoseconds / 1e9
                ey_diverge_threshold = max(0.150, self.initial_ey + 0.060)
                if elapsed > 3.0 and abs(e_y) > ey_diverge_threshold:
                ...
                # 3. Adaptive Lookahead (Multi-stage Virtual Carrot: Far -> Wide, Close -> Tight)
                ...
                # Lateral lookahead target point on docking centerline
                y_target = float(np.clip(e_y, -0.15, 0.15))
                x_target = float(L)
                alpha_lateral = math.atan2(y_target, x_target)

                # 4. Decoupled Velocity & Steering Control (Pure Pursuit + Independent Heading)
                # A. Velocity Scaling (Max 5.0 cm/s, down to 1.5 cm/s on large errors)
                v_base = 0.050
                v_min = 0.015
                ey_scale = 1.0 / (1.0 + 5.0 * abs(e_y))
                theta_scale = 1.0 / (1.0 + 2.0 * abs(e_theta)) if e_theta is not None else 1.0
                v_x = float(np.clip(v_base * ey_scale * theta_scale, v_min, v_base))

                # B. Decoupled Steering:
                # - omega_pp: Pure Pursuit lateral centerline convergence
                omega_pp = (2.0 * v_x * math.sin(alpha_lateral)) / L
                # - omega_heading: Independent proportional restorative alignment to desk normal
                k_theta = 0.40
                omega_heading = float(k_theta * e_theta) if e_theta is not None else 0.0

                w_raw = omega_pp + omega_heading
                w_z = float(np.clip(w_raw, -0.20, 0.20))

                twist.linear.x = v_x
                twist.angular.z = w_z
                self.carrot_x_b = x_target
                self.carrot_y_b = y_target
                self.get_logger().info(
                    f">>> [CARROT_ALIGN] Dist={self.desk_distance*100:.1f}cm, ey={e_y*100:+.1f}cm, yaw={math.degrees(e_theta):+.1f}°, "
                    f"vx={v_x*100:.1f}cm/s, wz={w_z:+.2f} (pp={omega_pp:+.2f}, hdg={omega_heading:+.2f}), L={L*100:.0f}cm",
                    throttle_duration_sec=0.8
                )
            else:
                # Marker momentarily lost while in CARROT_ALIGN:
                # If already close (<= 33cm) and was well centered, commence CRAWL_CONTACT
                if self.desk_distance is not None and self.desk_distance <= 0.33 and abs(self.target_lateral_offset) <= 0.020:
        ...
        # ----------------------------------------------------------------------
        # STATE: STANDOFF_SETTLE (Phase 1: Fine rotate to marker -> Phase 2: Halt >= 1.0s & statically evaluate error)
        # ----------------------------------------------------------------------
            ...
            # Phase 1: Fine in-place rotation to bring marker to center of image
            if self.settle_substate == 'FINE_ROTATE':
                if abs(e_theta) > math.radians(2.0) and elapsed < 3.0:
                    w_z = math.copysign(min(0.18, max(0.05, 0.60 * abs(e_theta))), e_theta)
                    twist.angular.z = float(w_z)
                    self.settle_stop_start_time = None
                    self.get_logger().info(
                        f">>> [STANDOFF_SETTLE] Fine rotating to marker: yaw_err={math.degrees(e_theta):+.1f}°, wz={w_z:+.2f}rad/s",
                        throttle_duration_sec=0.5
                    )
                else:
                    # Fine rotation finished (heading aligned <= 2.0 deg) -> Full Stop and begin STATIC_WAIT
                    twist.angular.z = 0.0
                    self.stop_robot()
                    self.settle_substate = 'STATIC_WAIT'
                    self.settle_stop_start_time = now
                    self.dual_align_stable_count = 0
                    self.get_logger().info(">>> [STANDOFF_SETTLE] Fine rotation finished. Halting all motors for >= 1.0s static evaluation...")
                    return

            # Phase 2: Completely stationary for >= 1.0s before evaluating error
                ...
                if stopped_elapsed < 1.0:
                    self.get_logger().info(
                        f">>> [STANDOFF_SETTLE] Stationary wait for vibration settling ({stopped_elapsed:.1f}s / 1.0s)...",
                        throttle_duration_sec=0.3
                    )
                    return

                # Statically evaluate error after >= 1.0s complete standstill
                aligned_ok = (abs(e_y) <= 0.025 and abs(e_theta) <= math.radians(3.0))
                if aligned_ok:
                    self.dual_align_stable_count += 1
                    if self.dual_align_stable_count >= 3:
                        self.locked_docking_yaw = self.get_robot_yaw()
                        self.get_logger().info(
                            f">>> [STANDOFF_SETTLE] Statically verified after {stopped_elapsed:.1f}s stationary pause! "
                            f"(ey={e_y*100:+.1f}cm <= 2.5cm, yaw={math.degrees(e_theta):+.1f}° <= 3°). "
                ...
                # If still significantly unaligned after 2.5s stationary evaluation:
                if stopped_elapsed > 2.5:
                    self.backup_reason = f"정지 계측(1초 정지 후) 오차 초과 (ey={e_y*100:+.1f}cm > 2.5cm, yaw={math.degrees(e_theta):+.1f}° > 3°)"
        ...
        # ----------------------------------------------------------------------
        # STATE: ALIGN_PARALLEL (In-place rotation to lock heading strictly parallel)
        # ----------------------------------------------------------------------
            ...
            if e_theta is not None and e_y is not None:
                aligned = (abs(e_theta) <= self.align_yaw_deadband) and (abs(e_y) <= 0.020)
                if aligned:
                    self.dual_align_stable_count += 1
                    twist.angular.z = 0.0
                    if self.dual_align_stable_count >= self.dual_align_stable_required:
                        self.locked_docking_yaw = self.get_robot_yaw()
                        self.get_logger().info(
                            f">>> [ALIGN_PARALLEL] Both Centerline & Heading Locked! (ey={e_y*100:+.1f}cm, yaw={math.degrees(e_theta):+.1f}°, mode={self.docking_alignment_mode}). "
                            f"Commencing CRAWL_CONTACT."
                        )
                        self.stop_robot()
                        self.transition_to('CRAWL_CONTACT')
                        return
                    ...
                    self.dual_align_stable_count = 0
                    if abs(e_theta) <= self.align_yaw_deadband:
                        w_z = 0.0
                    else:
                        w_z = math.copysign(min(0.20, max(0.06, 0.70 * abs(e_theta))), e_theta)
                    twist.angular.z = float(w_z)

            elapsed = (now - self.state_start_time).nanoseconds / 1e9
            if elapsed > 6.0:
                # If lateral offset is still too large (> 2.5cm), do not blindly crawl into desk!
                if e_y is not None and abs(e_y) > 0.025:
                    self.backup_reason = f"평행 정렬 시간 초과 (편차 미해소: {e_y*100:+.1f}cm > 2.5cm)"
                    self.get_logger().warn(
                        f">>> [ALIGN_PARALLEL] {self.backup_reason}. Backing up to 48cm to re-align cleanly..."
                    )
                    self.transition_to('BACKUP_STANDOFF')
                    return
                else:
                    self.locked_docking_yaw = self.get_robot_yaw()
                    self.get_logger().warn(">>> Parallel alignment time limit (6s). Locking heading and proceeding to CRAWL_CONTACT.")
                    self.transition_to('CRAWL_CONTACT')
                    return
        ...
        # ----------------------------------------------------------------------
            ...
            # 2. Case 4 Failure Guard: Timeout (5.0s) without physical bumper contact -> BACKUP_RETRY
            elapsed = (now - self.state_start_time).nanoseconds / 1e9
            if elapsed > 5.0:
                self.backup_reason = "최종 접근 5.0초 동안 범퍼 미접촉 (타임아웃)"
                self.get_logger().warn(">>> [CASE 4] Terminal crawl timeout (5.0s) without bumper contact. Initiating BACKUP_RETRY (25cm reverse)...")
                self.transition_to('BACKUP_RETRY')
                return

            # 4. Gyro Heading Lock Straight Crawl: drive forward holding locked_docking_yaw
            twist.linear.x = max(0.035, self.min_crawl_speed)
    ...
    def render_and_publish_visualization(self, color_img, corners_img, rvec, tvec, depth_m=None):
        try:
            ...
            # 2. Top Dashboard Banner (Ultra-lightweight direct rectangle, 0 copy)
            hud_h = 64
            cv2.rectangle(vis, (0, 0), (vis_w, hud_h), (18, 20, 24), -1)
            cv2.line(vis, (0, hud_h), (vis_w, hud_h), (50, 60, 70), 1)
            ...
            # Row 1 (Y=18): State and Bumper
            state_str = f"STATE: {self.state}"
            if self.state == 'NAV2_READY':
                state_str = "STATE: NAV2_READY"
            elif self.docking_routine_start_time is not None:
                elapsed = (self.get_clock().now() - self.docking_routine_start_time).nanoseconds / 1e9
                rem = max(0.0, self.docking_timeout_sec - elapsed)
                state_str += f" [{rem:3.0f}s]"
            cv2.putText(vis, state_str, (8, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.44, state_color, 1, cv2.LINE_AA)

            bumper_str = "BUMP: CONTACT!" if self.contact_detected else "BUMP: CLEAR"
            bumper_col = (0, 0, 255) if self.contact_detected else (180, 180, 180)
            cv2.putText(vis, bumper_str, (vis_w - 115, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.40, bumper_col, 1, cv2.LINE_AA)

            # Row 2 & 3: Target Distance, Heading Error, Lateral Error, Retry count
            e_x, e_y, e_theta = self.compute_docking_errors()
            if self.desk_distance is not None:
                ...
                d_col = (0, 255, 0) if dist_cm <= 7.0 else ((0, 255, 255) if dist_cm <= 25.0 else (240, 240, 240))
                y_col = (0, 255, 0) if abs(yaw_deg) <= 2.5 else (0, 200, 255)
                e_col = (0, 255, 0) if abs(ey_cm) <= 1.5 else (0, 200, 255)

                dist_text = f"DIST: {dist_cm:4.1f}cm (Aim:{self.target_clearance*100:.0f}cm)"
                ey_text = f"ey: {ey_cm:+4.1f}cm"
                fit_src = getattr(self, 'docking_alignment_mode', getattr(self, 'plane_fit_source', 'PNP'))
                yaw_text = f"YAW: {yaw_deg:+4.1f}deg [{fit_src}]"
                retry_text = f"TRY:{self.retry_count}/{self.max_retries}"

                cv2.putText(vis, dist_text, (8, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.38, d_col, 1, cv2.LINE_AA)
                cv2.putText(vis, ey_text, (vis_w - 115, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.38, e_col, 1, cv2.LINE_AA)
                cv2.putText(vis, yaw_text, (8, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.38, y_col, 1, cv2.LINE_AA)
                cv2.putText(vis, retry_text, (vis_w - 115, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (200, 200, 200), 1, cv2.LINE_AA)
            else:
                if self.state == 'NAV2_READY':
                    mode_label = "NAV2 ACTIVE | ASTRA S STREAMING"
                else:
                    mode_label = f"SEARCHING ARUCO ID {self.marker_id}..."
                cv2.putText(vis, mode_label, (8, 42), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (120, 200, 255), 1, cv2.LINE_AA)
    ...
    # ==========================================================================
    # Legacy OS30A 3D Depth Servoing (Depth Edge Mode)
    # ==========================================================================
    def process_depth_for_desk_edge(self, depth_m: np.ndarray):
        ...
        table_mask = (z_b >= self.table_z_min) & (z_b <= self.table_z_max) & (x_b >= 0.145) & (x_b < 1.40) & (np.abs(y_b) <= 0.40)
        if np.count_nonzero(table_mask) < 15:
            return
        ...
        for i in range(len(y_bins) - 1):
            bin_m = (y_b >= y_bins[i]) & (y_b < y_bins[i + 1]) & table_mask
            if np.count_nonzero(bin_m) >= 2:
                front_x = float(np.percentile(x_b[bin_m], 10))
                edge_x.append(front_x)
                edge_y.append(0.5 * (y_bins[i] + y_bins[i + 1]))

        if len(edge_x) >= 5:
            p = np.polyfit(edge_y, edge_x, 1)
            slope_m, intercept_c = p[0], p[1]
            dist = float(np.median(edge_x)) - self.reference_x
            yaw_err = math.atan(slope_m)
            self.desk_distance = dist
            self.desk_yaw_error = yaw_err
            self.render_and_publish_visualization(self.last_color_img, None, None, None, depth_m=depth_m)
    ...
    def transition_to(self, new_state: str):
        ...
        if new_state in ('CRAWL_CONTACT', 'FINAL_APPROACH'):
            self._stall_check_dist = None
            self._stall_start_time = None

        if new_state in ('CARROT_ALIGN', 'CARROT_PURSUIT_ALIGN'):
            self.initial_ey = None

        if new_state in ('STANDOFF_SETTLE', 'ALIGN_PARALLEL', 'VERIFY_DUAL_ALIGN', 'CARROT_ALIGN'):
            self.dual_align_stable_count = 0
        ...
        if new_state in ('BACKUP_STANDOFF', 'BACKUP_PREPARE', 'BACKUP_ALIGN_PREPARE'):
            self.backup_substate = 'ALIGN'
        else:
    ...
    def handle_start_docking(self, request, response):
        ...
        if not self.dual_markers_visible:
            msg = "도킹 시작 실패: 마커 2개(ID 0, ID 1)가 모두 카메라 시야에 인식되어야 합니다."
            self.get_logger().warn(f">>> [/start_docking] Rejected: {msg}")
            response.success = False
            response.message = msg
            return response

        # Strict Safety Gate 2: Distance to target must be within 80cm (0.80m)
        if self.desk_distance is None or self.desk_distance > 0.80:
            dist_val = f"{self.desk_distance*100:.1f}cm" if self.desk_distance is not None else "미인식"
            msg = f"도킹 시작 실패: 마커와의 거리가 80cm 이내여야 합니다 (현재: {dist_val})."
            self.get_logger().warn(f">>> [/start_docking] Rejected: {msg}")
            response.success = False
            response.message = msg
            return response

        self.get_logger().info(f">>> [/start_docking] Dual markers verified (d={self.desk_distance*100:.1f}cm <= 80cm). Starting docking routine!")
        self.contact_latched = False
        self.ensure_motor_power()
        self.retry_count = 0
        self.initial_ey = None
    ...
    def get_robot_yaw(self):
        ...
        for target_frame in ('odom', 'map'):
            try:
                t = self.tf_buffer.lookup_transform(target_frame, 'base_footprint', rclpy.time.Time())
                q = t.transform.rotation
                siny_cosp = 2.0 * (q.w * q.z + q.x * q.y)
                cosy_cosp = 1.0 - 2.0 * (q.y * q.y + q.z * q.z)
